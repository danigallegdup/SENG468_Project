...
userService.post("/createStock", async (req, res) => {
    ...
    if (!stockName) return res.status(400).json({ error: "Stock name required" });
    ...
    try {
        ...
        if (stockExists) return res.status(409).json({ error: "Duplicate stock found" });

        const result = await stocksCollection.insertOne({ stockName });
        res.json({ stockId: result.insertedId, stockName });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// getStockPortfoilio endpoint
...
userService.post("/addStockToUser", async (req, res) => {
    ...
    try {
        ...
        if (!stockExists) return res.status(404).json({ error: "Stock not found" });
        ...
        if (stockAlreadyInPortfolio) {
            ...
            res.json({ message: "Stock added to user portfolio" });
        }
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
